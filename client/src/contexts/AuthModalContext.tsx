...
export function AuthModalProvider({ children }: { children: React.ReactNode }) {
  ...
  const [actionType, setActionType] = useState<
    "cart" | "wishlist" | "checkout" | "tickets" | null
  >(null);
  ...
  const openAuthModal = (
    ...
    const normalizedPendingAction: PendingAuthAction | undefined = pendingAction
      ? {
          ...pendingAction,
          type:
            pendingAction.type || (newActionType as PendingAuthAction["type"]),
    ...
    const safeRedirectTo = sanitizeInternalRedirect(
      normalizedPendingAction?.redirectTo
    );
