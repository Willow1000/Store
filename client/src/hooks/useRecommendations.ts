...
export function useRecommendations(userId?: string | null) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      RECOMMENDATION_PROFILE_CHANGED_EVENT,
      onProfileChanged as EventListener
    );
  ...
  const track = useCallback(
    (event: Omit<RecommendationSignalEvent, "userId">) => {
  ...
  return useMemo(
    () => ({
      ...
      track,
      rank,
      recommendedProducts: (products: Product[], limit?: number) =>
        getRecommendedProducts(products, profile, limit),
      personalizedDeals: (
        products: Product[],
        limit?: number,
        searchTerm?: string
      ) => getPersonalizedDeals(products, profile, limit, searchTerm),
      continueShoppingProducts: (products: Product[], limit?: number) =>
        getContinueShoppingProducts(products, profile, limit),
      similarVehicleParts: (
        products: Product[],
        currentProduct: Product,
        limit?: number
      ) => getSimilarVehicleParts(products, currentProduct, profile, limit),
      completeRepairProducts: (
        products: Product[],
        currentProduct: Product,
        limit?: number
      ...
      analytics: (products?: Product[]) =>
        getRecommendationAnalytics(profile, products),
    }),
    [profile, rank, track]
  );
}
