...
const GOOGLE_MAPS_PLACE_LINK =
  "https://maps.google.com/?q=Nijverheidsweg+27,+Heinenoord,+Netherlands";
// Explicit opt-in keeps local/dev clean when API billing/key restrictions are not ready.
const MAPS_TOGGLE = String(import.meta.env.VITE_GOOGLE_MAPS_ENABLED || "")
  .trim()
  .toLowerCase();
const MAPS_ENABLED = MAPS_TOGGLE
  ? MAPS_TOGGLE === "true"
  : MAPS_API_KEY.length > 0;
  ...
  return new Promise((resolve, reject) => {
    ...
    if (existing) {
      ...
      existing.addEventListener(
        "error",
        () => reject(new Error("Failed to load Google Maps locator script")),
        {
          once: true,
        }
      );
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        ...
        const locator = document.createElement(
          "gmpx-store-locator"
        ) as StoreLocatorElement;
        if (MAPS_MAP_ID) {
    ...
    return (
      <div className="h-full w-full rounded-lg border border-gray-200 bg-gray-50 p-4 flex flex-col justify-center">
        ...
        <p className="text-sm text-gray-600">
          Nijverheidsweg 27, Heinenoord, Netherlands
        </p>
        {loadError ? (
