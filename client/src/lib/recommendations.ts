...
export const RECOMMENDATION_PROFILE_CHANGED_EVENT =
  "motorvault:recommendation-profile-changed";
...
function canonicalVehicleBrand(value: string): string {
  ...
  if (normalized === "mercedes benz" || normalized === "mercedes")
...
function canonicalCategory(value: string): string {
  ...
  for (const [category, aliases] of Object.entries(CATEGORY_ALIASES)) {
    if (
      normalizeKey(category) === normalized ||
...
function writeStoredProfile(profile: InterestProfile) {
  ...
  try {
    localStorage.setItem(
      readProfileKey(profile.scope),
      JSON.stringify(profile)
    );
    window.dispatchEvent(
      new CustomEvent(RECOMMENDATION_PROFILE_CHANGED_EVENT, { detail: profile })
    );
  } catch {
...
function mergeProfiles(
  primary: InterestProfile,
  secondary: InterestProfile
): InterestProfile {
  ...
  merged.recentSearches = [
    ...primary.recentSearches,
    ...secondary.recentSearches,
  ]
    .sort((a, b) => new Date(b.at).getTime() - new Date(a.at).getTime())
    .slice(0, MAX_RECENT_SEARCHES);
  merged.recentlyViewedProductIds = mergeUnique(
    primary.recentlyViewedProductIds,
    secondary.recentlyViewedProductIds
  );
  merged.cartProductIds = mergeUnique(
    primary.cartProductIds,
    secondary.cartProductIds
  );
  merged.purchasedProductIds = mergeUnique(
    primary.purchasedProductIds,
    secondary.purchasedProductIds
  );
  merged.wishlistedProductIds = mergeUnique(
    primary.wishlistedProductIds,
    secondary.wishlistedProductIds
  );
...
export function getRecommendationProfile(
  ...
  if (userId) {
    ...
    if (
      hasGuestSignals &&
      profile.lastMergedGuestUpdatedAt !== guest.updatedAt
    ) {
...
function inferKnownBrand(text: string, brands: string[]): string | undefined {
  ...
  const match = sortedBrands.find(brand =>
    normalized.includes(` ${normalizeKey(brand).replace(/-/g, " ")} `)
  );
  return match;
}

function inferProductBrand(
  text: string,
...
function inferModel(text: string, brand?: string): string | undefined {
  ...
  const brandIndex = brand
    ? tokens.findIndex(
        token =>
          normalizeKey(token) === normalizeKey(brand) ||
          (brand === "Volkswagen" && normalizeKey(token) === "vw")
      )
    : -1;
  ...
  const candidate =
    brandIndex >= 0
      ? tokens
          ...
          .find(
            token =>
              /^[a-z]?\d{1,3}[a-z0-9-]*$/i.test(token) ||
              /^[a-z]-?class$/i.test(token)
          )
...
export function inferSearchIntent(
  ...
  const vehicleBrand = vehicleBrandMatch
    ? canonicalVehicleBrand(vehicleBrandMatch)
    : undefined;
...
function applySearchSignal(
  ...
  profile.recentSearches = [
    ...
    .filter(
      (entry, index, list) =>
        list.findIndex(
          other => normalizeKey(other.raw) === normalizeKey(entry.raw)
        ) === index
    )
    .slice(0, MAX_RECENT_SEARCHES);

  if (intent.category)
    addWeight(profile.categoryWeights, intent.category, 44 * multiplier);
  if (intent.productBrand)
    addWeight(profile.brandWeights, intent.productBrand, 28 * multiplier);
...
function applyProductSignal(
  ...
  amount: number,
  eventType: RecommendationEventType,
  explicitProductId?: string | number | null
) {
  ...
  if (product?.brand || product?.model || category) {
    updateVehicleContext(
      ...
      {
        brand: product?.brand
          ? canonicalVehicleBrand(product.brand)
          : profile.vehicleContext?.brand,
        model: product?.model
          ? String(product.model)
          : profile.vehicleContext?.model,
  ...
  if (productId && eventType === "purchase") {
    profile.purchasedProductIds = pushUnique(
      profile.purchasedProductIds,
      productId
    );
  }
  if (productId && eventType === "wishlist_add") {
    profile.wishlistedProductIds = pushUnique(
      profile.wishlistedProductIds,
      productId
    );
  }
  if (productId && eventType === "wishlist_remove") {
    profile.wishlistedProductIds = removeValue(
      profile.wishlistedProductIds,
      productId
    );
  }
...
export function trackRecommendationEvent(
  event: RecommendationSignalEvent
): InterestProfile {
  ...
  if (event.searchTerm) {
    applySearchSignal(
      profile,
      event.searchTerm,
      event.products?.filter(Boolean) as Partial<Product>[] | undefined
    );
  }
  ...
  if (event.category) {
    addWeight(
      ...
      event.category,
      eventType === "category_view" ? 14 : 6
    );
    updateVehicleContext(
      profile,
      { category: canonicalCategory(event.category) },
      35
    );
  }

  const products = event.products?.filter(Boolean) as
    | Partial<Product>[]
    | undefined;
...
export function sendRecommendationTrackingEvent(
  event: RecommendationSignalEvent,
  profile = getRecommendationProfile(event.userId)
) {
...
function productMatchesCategory(
  ...
  if (
    related.some(
      relatedCategory =>
        normalizeKey(productCategory).includes(normalizeKey(relatedCategory)) ||
        findAliasMatch(text, [
          relatedCategory,
          ...(CATEGORY_ALIASES[relatedCategory] || []),
        ])
    )
  ) {
    return "related";
...
export function calculateDealScore(product: Partial<Product>): number {
  ...
  const discountPercent =
    original > price && price > 0 ? ((original - price) / original) * 100 : 0;
...
function calculateSearchScore(
  ...
  if (
    intent.productBrand &&
    intent.category === "Tires" &&
    PREMIUM_TIRE_BRANDS.some(brandName =>
      normalizeKey(text).includes(normalizeKey(brandName))
    )
  ) {
    score += normalizeKey(text).includes(normalizeKey(intent.productBrand))
      ? 40
      : 28;
  }
...
function calculateVehicleAffinity(
  product: Partial<Product>,
  profile: InterestProfile
): number {
  let score = 0;
  ...
  if (
    context?.brand &&
    normalizeKey(text).includes(normalizeKey(context.brand))
  ) {
    score += 25 * Math.min(1, context.confidence / 100);
  }
  if (
    context?.model &&
    normalizeKey(text).includes(normalizeKey(context.model))
  ) {
...
function getLatestIntentSearchTerm(
  ...
  if (
    ...
    Date.now() - trackedAt > LATEST_INTENT_WINDOW_MS
  ) {
    return undefined;
  }
...
export function scoreProduct(
  ...
  const searchTerm =
    explicitSearchTerm || getLatestIntentSearchTerm(profile) || "";
  const intent = searchTerm
    ? inferSearchIntent(searchTerm, options.products)
    : undefined;
  ...
  const breakdown: SmartScoreBreakdown = {
    searchRelevance:
      calculateSearchScore(product, intent, profile) * searchIntentMultiplier,
  ...
  return products
    ...
    .sort((a, b) => {
      ...
      if (options.priceDirection) {
        ...
        return options.priceDirection === "asc"
          ? aPrice - bPrice
          : bPrice - aPrice;
      }
...
export function getPersonalizedDeals(
  ...
  return products
    ...
    .map((product, index) => ({
      ...
      index,
      score:
        scoreProduct(product, { profile, searchTerm }).score +
        calculateDealScore(product) * 1.2,
    }))
...
export function getContinueShoppingProducts(
  products: Product[],
  profile: InterestProfile,
  limit = 10
): Product[] {
  ...
  const recentCategories = new Set(
    recent.map(product => normalizeKey(product.category_name))
  );
  ...
  return [...recent, ...rankProducts(categoryMatches, { profile })].slice(
    0,
    limit
  );
}

export function getSimilarVehicleParts(
  products: Product[],
  ...
  const currentBrand =
    currentProduct.brand || profile.vehicleContext?.brand || "";
  const currentModel =
    currentProduct.model || profile.vehicleContext?.model || "";
  ...
  return products
    ...
    .map((product, index) => {
      const sameBrand =
        currentBrand &&
        normalizeKey(productText(product)).includes(normalizeKey(currentBrand));
      const sameModel =
        currentModel &&
        normalizeKey(productText(product)).includes(normalizeKey(currentModel));
      const differentCategory =
        normalizeKey(getProductCategory(product)) !==
        normalizeKey(currentCategory);
      const related = (RELATED_CATEGORIES[currentCategory] || []).some(
        category => productMatchesCategory(product, category) !== "none"
      );
      const score =
        (sameBrand ? 70 : 0) +
        (sameModel ? 55 : 0) +
...
export function getCompleteRepairProducts(
  products: Product[],
  currentProduct: Product,
  profile: InterestProfile,
  limit = 10
  ...
  return products
    .filter(product => product.id !== currentProduct.id)
    .map((product, index) => {
      const relatedScore = relatedCategories.reduce(
        (sum, category, relatedIndex) => {
          return (
            sum +
            (productMatchesCategory(product, category) !== "none"
              ? 60 - relatedIndex * 4
              : 0)
          );
        },
      ...
      const vehicleScore =
        ...
        normalizeKey(productText(product)).includes(
          normalizeKey(currentProduct.brand)
        )
          ? 28
          : 0;
      const titleTokenScore = tokenize(text).some(token =>
        normalizeKey(productText(product)).includes(token)
      )
        ? 8
        : 0;
      return {
        ...
        score:
          ...
          vehicleScore +
          titleTokenScore +
          scoreProduct(product, { profile }).score * 0.2,
      };
    })
...
export function getFrequentlyBoughtTogetherProducts(
  ...
  return products
    ...
    .map((product, index) => {
      ...
      const categoryScore = relatedCategories.some(
        category => productMatchesCategory(product, category) !== "none"
      )
        ? 48
        : 0;
      const vehicleScore =
        currentProduct.brand &&
        product.brand &&
        normalizeKey(product.brand) === normalizeKey(currentProduct.brand)
          ? 24
          : 0;
      return {
        product,
        ...
        score:
          ...
          categoryScore +
          vehicleScore +
          calculateDealScore(product) * 0.4,
      };
    })
...
export function getRecommendationAnalytics(
  ...
  const conversions =
    profile.eventCounts.recommendation_conversion ||
    profile.eventCounts.purchase ||
    0;
