 ...
 */
export async function redirectToStripeCheckout(input: StripeCheckoutInput): Promise<void> {
  try {
    const response = await trpcClient.stripe.payments.createCheckoutSession.mutate({
      userId: input.userId,
      email: input.email,
