...
export type PendingAuthActionType =
  | "cart"
  | "wishlist"
  | "checkout"
  | "tickets";
...
async function upsertCartItem(
  ...
  if (insertError) {
    if (insertError.code === "23505") {
      ...
      if (!updateItem?.id)
        throw new Error("Cart item not found after concurrent insert");
...
export function waitForCartMigration(): Promise<void> {
  ...
  const w = window as any;
  if (w.__cartMigrationPromise)
    return w.__cartMigrationPromise as Promise<void>;
...
async function mergeGuestWishlistWithUserWishlist(
  userId: string
): Promise<void> {
  const raw =
  ...
  try {
    ...
    if (action) {
      const pendingProductId =
        typeof action.productId === "string" ? action.productId : null;
