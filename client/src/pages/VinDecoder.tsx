...
import { trpc } from '@/lib/trpc';import {
  ...
  ClipboardList,
  CheckCircle2,
} from 'lucide-react';import './VinDecoder.css';
...
export default function VinDecoder() {
  ...
  const handleDecodeVin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.data) {
        ...
        // Add to history
        const newHistory = [vin.toUpperCase(), ...vinHistory.filter(v => v !== vin.toUpperCase())].slice(0, 10);
        setVinHistory(newHistory);
        try {
          localStorage.setItem('vinDecoder_history', JSON.stringify(newHistory));
        } catch {
      ...
      const message = error?.message || 'Failed to decode VIN. Please check the VIN and try again.';
      toast.error(message);
      setDecodedVehicle(null);
    } finally {
      setIsLoading(false);
  ...
  const handleFindProducts = async () => {
    ...
    try {
      ...
      if (res.data) {
        ...
        toast.success(`Found ${res.data.totalMatches || 0} products matching the VIN`);
      }
    } catch (err) {
  ...
  const renderVehicleInfo = () => {
    ...
    const fieldsByPriority = [
      ...
      { label: 'Year', value: decodedVehicle.year, Icon: Calendar, priority: 1 },
      { label: 'Make', value: decodedVehicle.make, Icon: Building2, priority: 1 },
      { label: 'Model', value: decodedVehicle.model, Icon: Car, priority: 1 },
      { label: 'Trim', value: decodedVehicle.trim, Icon: Star, priority: 1 },

      // Tier 2: Body & Type
      { label: 'Body Class', value: decodedVehicle.bodyClass, Icon: Package, priority: 2 },
      { label: 'Vehicle Type', value: decodedVehicle.vehicleType, Icon: Truck, priority: 2 },

      // Tier 3: Engine Specifications
      { label: 'Engine Cylinders', value: decodedVehicle.engineCylinders, Icon: Wrench, priority: 3 },
      { label: 'Displacement', value: decodedVehicle.displacement, Icon: Zap, priority: 3 },
      { label: 'Fuel Type', value: decodedVehicle.fuelType, Icon: Fuel, priority: 3 },
      { label: 'Drive Type', value: decodedVehicle.driveType, Icon: Gauge, priority: 3 },

      // Tier 4: Manufacturing
      { label: 'Manufacturer', value: decodedVehicle.manufacturer, Icon: Building2, priority: 4 },
      { label: 'Plant Country', value: decodedVehicle.plantCountry, Icon: Globe2, priority: 4 },
    ];
    ...
    return (
      <div className="vin-decoder-result">
        <div className="result-header">
          ...
          <p className="vin-display">VIN: <code>{vin}</code></p>
        </div>
        ...
        {getSectionFields(1).length > 0 && (
          <div className="info-section">
            <div className="section-title">
              <span className="title-icon"><Car className="w-6 h-6" /></span>
            ...
            <div className="section-grid tier-1">
              {getSectionFields(1).map(({ label, value, Icon }) => (
                <div key={label} className="elegant-info-card">
                  <div className="card-icon"><Icon className="w-6 h-6" /></div>
        ...
        {getSectionFields(2).length > 0 && (
          <div className="info-section">
            <div className="section-title">
              <span className="title-icon"><ClipboardList className="w-6 h-6" /></span>
        ...
        {getSectionFields(3).length > 0 && (
          <div className="info-section">
            <div className="section-title">
              <span className="title-icon"><Zap className="w-6 h-6" /></span>
        ...
        {getSectionFields(4).length > 0 && (
          <div className="info-section">
            <div className="section-title">
              <span className="title-icon"><Building2 className="w-6 h-6" /></span>
        ...
        {Object.entries(decodedVehicle).some(
          ...
          <div className="info-section">
            <div className="section-title">
              <span className="title-icon"><BarChart3 className="w-6 h-6" /></span>
              ...
              {Object.entries(decodedVehicle).map(([key, value]) => {
                ...
                return (
                  <div key={key} className="elegant-info-card">
                    <div className="card-icon"><MapPin className="w-6 h-6" /></div>
                    <div className="card-content">
  ...
  return (
    <>
      <SEOHead
        ...
        keywords={["VIN decoder", "vehicle lookup", "VIN lookup", "compatible parts", "motor parts"]}
      />
      <div className="vin-decoder-container">
      <div className="vin-decoder-wrapper">
        <h1>VIN Decoder</h1>
        <p className="description">
          Enter a Vehicle Identification Number (VIN) to decode vehicle information. A VIN is a 17-character code
        ...
        <form onSubmit={handleDecodeVin} className="vin-decoder-form">
          ...
          <button type="submit" disabled={isLoading || !vin.trim()} className="decode-button">
        ...
        {vinHistory.length > 0 && (
          ...
          <div className="vin-product-search">
            ...
            >
              {productsLoading ? 'Searching products...' : 'Find Compatible Products'}
            </button>
          </div>
        ...
        >
          <div className="results-header">
            ...
            <p className="sr-only" aria-hidden={false}>{productsTotal} products found</p>
          </div>

          {productsLoading && <p>Searching for compatible products…</p>}

          {!productsLoading && products.length === 0 && (
            <p className="no-results">No matching products found for this VIN.</p>
          )}
                  ...
                  <a href={`/product/${p.id}`} className="product-link">
                    <div className="product-thumb">
                      {p.images && p.images[0] ? (
                        ...
                        <div className="product-placeholder" aria-hidden>Image</div>
                      )}
                    </div>
                    <div className="product-meta">
                      <div className="product-name">{p.name}</div>
                      <div className="product-price">${p.price}</div>
                      <div className="product-snippet">{(p.description || '').slice(0, 120)}</div>
                    </div>
                  </a>
                </li>
              ))}
        ...
        <div className="info-box">
          ...
          <ul>
            <li>A VIN (Vehicle Identification Number) is a unique 17-character code assigned to each vehicle.</li>
            <li>The VIN contains encoded information about the vehicle's manufacturer, model, year, and features.</li>
            <li>VINs follow the ISO 3779 standard and are displayed on the vehicle's dashboard and registration.</li>
            <li>This decoder provides information such as year, make, model, engine type, and more.</li>
          </ul>
        </div>
      </div>
    </div>
