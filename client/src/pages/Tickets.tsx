...
export default function TicketsPage() {
  ...
  const setSelectedFilesFromInput = (files: File[]) => {
    ...
    if (files.length > 5) {
      setFileWarning(
        "You may select up to 5 files; only the first 5 will be kept."
      );
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: any = {
        ...
        contactPhone:
          sanitizePhone(buildInternationalPhoneNumber(country, phone), 24) ||
          null,
      ...
      if (!res.ok) {
        throw new Error(
          json?.error || `Failed to create ticket (${res.status})`
        );
      }

      setMessage(
        `Ticket created: ${json.ticket?.referencecode || json.ticket?.referenceCode}`
      );
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      <form
        ...
        aria-labelledby="tickets-form-heading"
      >
        <h2 id="tickets-form-heading" className="sr-only">
        ...
        <div>
          ...
          <input
            ...
            value={email}
            onChange={e => setEmail(sanitizeEmail(e.target.value, 255))}
          />
        </div>
        <div>
          ...
          <div className="grid gap-2 sm:grid-cols-[180px_minmax(0,1fr)]">
            ...
            <input
              ...
              onChange={e =>
                setPhone(
                  formatLocalPhoneNumber(
                    normalizeLocalPhoneDigits(e.target.value, country, 15),
                    country
                  )
                )
              }
        ...
        <div>
          ...
          <textarea
            ...
            value={description}
            onChange={e =>
              setDescription(sanitizeMultilineTextInput(e.target.value, 5000))
            }
          />
        </div>
        <div>
        ...
        <div>
          ...
          >
            ...
            <span className="text-xs text-slate-600">
              Tap or click to select images or PDF files from your device.
            </span>
            <span className="text-xs text-slate-500">
              Up to 5 attachments will be kept.
            </span>
          </label>
          <input
            ref={fileInputRef}
          ...
          />
          <div id="attachments-help" className="text-xs text-gray-500">
            You may select images or PDF files. These files stay on your device
            and are not submitted.
          </div>
          {fileWarning && (
            <div className="text-xs text-yellow-600 mt-1">{fileWarning}</div>
          )}
      ...
      {message && (
        <div role="status" aria-live="polite" className="mb-4 p-3 bg-gray-100">
          {message}
        </div>
      )}
