...
import {
  ...
  parseMetaCouponPercent,
  parseMetaCheckoutParams,
...
const PRELOAD_LOCATION_COUNTRIES = [
  "DE",
  "FR",
  "IT",
  "ES",
...
let locationDataModulePromise: Promise<
  typeof import("country-state-city")
> | null = null;
let countryOptionsCache: CountryOption[] | null = null;

async function ensureLocationDataModule(): Promise<
  typeof import("country-state-city")
> {
...
function normalizeRegionCode(country: string, value: string): string {
  ...
  const match = states.find(
    state => state.name.toLocaleLowerCase() === normalized
  );
...
const getStateOptions = (country: string): StateOption[] => {
  ...
  if (states.length > 0) {
    const cityStateCodes =
      countryCode === "US"
        ? new Set<string>()
        : getCountryCityStateCodes(countryCode);
    const statesWithCityData =
      cityStateCodes.size > 0
        ? states.filter(state => cityStateCodes.has(state.isoCode))
        : [];
    const selectableStates =
      statesWithCityData.length > 0 ? statesWithCityData : states;
    const options = [
  ...
  const countryName =
    locationDataModule?.Country.getCountryByCode(countryCode)?.name;
...
const usesManualLocationFields = (country: string): boolean => {
  ...
  return (
    !hasAnyStateCityData(countryCode) &&
    getCountryCityOptions(countryCode).length === 0
  );
};
...
function getCountryCityOptions(country: string): string[] {
  ...
  const cities = uniqueSortedCityNames(
    locationDataModule.City.getCitiesOfCountry(countryCode) || []
  );
...
function getStateCityOptions(country: string, state: string): string[] {
  ...
  if (!countryCode || !stateCode || stateCode === COUNTRY_REGION_VALUE)
    return [];
  ...
  const cities = uniqueSortedCityNames(
    locationDataModule.City.getCitiesOfState(countryCode, stateCode) || []
  );
...
function hasAnyStateCityData(country: string): boolean {
  ...
  const hasCityData =
    Boolean(
      firstState &&
        getStateCityOptions(countryCode, firstState.isoCode).length > 0
    ) ||
    states.some(state =>
      getCountryCityStateCodes(countryCode).has(state.isoCode)
    );
...
export default function Checkout() {
  ...
  const [appliedCouponLabel, setAppliedCouponLabel] = useState<string | null>(
    null
  );
    ...
    setFormData(prev => {
      ...
      const shouldUseGeoCountry =
        (!prev.address &&
          !prev.state &&
          !prev.city &&
          (!prev.country || prev.country === DEFAULT_PHONE_COUNTRY)) ||
      ...
      const next: CheckoutFormData = {
        ...
        phoneCountry:
          ...
          (!prev.phoneCountry ||
            prev.phoneCountry === prev.country ||
            prev.phoneCountry === DEFAULT_PHONE_COUNTRY)
            ? geoCountry
            : prev.phoneCountry,
      };
      ...
      const stateExists = getStateOptions(geoCountry).some(
        state => state.value === normalizedState
      );
      if (!stateExists) {
        if (!prev.zip && geoZip) {
          next.zip = geoZip;
  ...
  useEffect(() => {
    ...
    const hydrateMetaCart = async () => {
      ...
      if (unavailable.length > 0) {
        toast.warning(
          "Some items were unavailable and were removed from checkout."
        );
      }
    };
  ...
  useEffect(() => {
    ...
    if (paymentState === "pending") {
      ...
      toast.error(
        `Payment is not completed yet${statusLabel}. Please complete payment and try again.`
      );
      setStep("review");
      return;
    }
    ...
    if (paymentState === "needs_auth") {
      ...
      clearPaymentParams();
      toast.error(
        "Please sign in to complete your order for this verified payment."
      );
      setStep("shipping");
      return;
    }
    ...
    if (paymentState === "success") {
      ...
      (async () => {
        try {
          ...
          const purchaseAmount = Number.isFinite(amountValue)
            ? Number((amountValue / 100).toFixed(2))
            : null;
      ...
      (async () => {
        try {
          const verification =
            await trpcClient.paystack.transactions.verify.query({ reference });
          const status = String(
            (verification as any)?.data?.status || ""
          ).toLowerCase();
          ...
          if (["ongoing", "pending", "processing", "queued"].includes(status)) {
            toast.error(
              `Payment is not completed yet (${status}). Please complete payment and try again.`
            );
            setStep("review");
            return;
          }
  ...
  useEffect(() => {
    ...
    setFormData(prev => ({
      ...prev,
      firstName:
        prev.firstName || sanitizeName(user.name?.split(" ")[0] || "", 60),
      lastName:
        prev.lastName ||
        sanitizeName(user.name?.split(" ").slice(1).join(" ") || "", 60),
  ...
  useEffect(() => {
    const loadDefaultAddress = async () => {
      ...
      setFormData(prev => {
        const savedCountry = String(
          data.country || prev.country || "US"
        ).toUpperCase();
        const shouldUseSavedCountry =
          !prev.address && !prev.state && !prev.city;
        ...
        return {
          ...
          country: shouldUseSavedCountry
            ? savedCountry
            : prev.country || savedCountry,
  ...
  useEffect(() => {
    ...
    const normalizedState = normalizeRegionCode(
      formData.country,
      formData.state
    );
  ...
  const resolvedOffer = trpc.offers.resolve.useQuery(
    {
      code: metaCoupon || "",
      subtotal: Math.round(cartSubtotalAmount * 100) / 100,
    },
    {
  ...
  const legacyCouponPercent = useMemo(
    () =>
      isMetaCheckout && !resolvedOffer.data
        ? parseMetaCouponPercent(metaCoupon)
        : 0,
  ...
  const orderSummary = useMemo(() => {
    ...
    const discountAmountValue =
      ...
      appliedOfferData?.discountAmount ??
      derivedOfferDiscountAmount;

    const couponLabelValue =
      appliedCouponLabel ??
      (finalAppliedOffer
  ...
  const trackConfirmedPurchase = (
    ...
    const purchaseItems =
      cartItems.length > 0 ? cartItems : readCheckoutSnapshot();
    ...
    const numItems = purchaseItems.reduce(
  ...
  const paymentMethods: PaymentMethod[] = [
    {
      ...
      icon: (
        <img
          src="data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 48 48%22%3E%3Crect fill=%23635BFF width=%2248%22 height=%2248%22/%3E%3Ctext x=%2724%22 y=%2726%22 text-anchor=%22middle%22 fill=%22white%22 font-size=%2216%22 font-weight=%22bold%22%3ES%3C/text%3E%3C/svg%3E"
          alt="Stripe logo"
  ...
  const handleShippingChange = (
    ...
    const sanitizedValue = (() => {
      if (name === "firstName" || name === "lastName")
        return sanitizeNameInput(value, 60);
      ...
      if (name === "state") {
        ...
        return manualLocationFields
          ? nextState
          : normalizeRegionCode(formData.country, nextState);
      }
      ...
      if (name === "phoneCountry")
        return sanitizeTextInput(value, 8).toUpperCase();
    ...
    if (name === "country") {
      setFormData(prev => ({
        ...
        phoneCountry:
          !prev.phoneCountry || prev.phoneCountry === prev.country
            ? sanitizedValue
            : prev.phoneCountry,
        state: "",
        city: "",
  ...
  const handlePhoneBlur = () => {
    setFormData(prev => ({
      ...prev,
      phone: formatLocalPhoneNumber(
        normalizeLocalPhoneDigits(prev.phone, prev.phoneCountry, 15),
        prev.phoneCountry
      ),
    }));
  };
  ...
  const validateShipping = () => {
    ...
    if (!usesManualLocationFields(formData.country)) {
      const availableStateOptions = getStateOptions(formData.country).filter(
        option => option.value
      );
      const selectedStateUsesManualCity = usesManualCityField(
        formData.country,
        formData.state
      );
      const availableCityOptions = getCityOptions(
        formData.country,
        formData.state
      );
      if (
        !availableStateOptions.some(option => option.value === formData.state)
      ) {
        errors.state = `${getStateLabel(formData.country)} must be selected from the list`;
      }
      if (
        !selectedStateUsesManualCity &&
        !availableCityOptions.includes(formData.city)
      ) {
    ...
    try {
      const normalized = (value: string | null | undefined) =>
        String(value || "")
          .trim()
          .toLowerCase();
      const nextAddressLine = composeAddressLine(
        sanitizeText(formData.address, 255),
        sanitizeText(formData.state, 32)
      );
      ...
      const { data: existingAddress, error: existingAddressError } =
        await supabase
          .from("addresses")
          .select(
            "id, country, city, address_line, postal_code, is_default, created_at"
          )
          .eq("user_id", user.id)
          .order("created_at", { ascending: false })
          .limit(1)
      ...
      if (!existingAddress) {
        ...
        toast.success(
          checkoutText(
            ...
            "Shipping address saved for future checkout"
          )
        );
        return true;
      }
      ...
      const sameAddress =
        normalized(existingAddress.country) ===
          normalized(nextAddressPayload.country) &&
        normalized(existingAddress.city) ===
          normalized(nextAddressPayload.city) &&
        normalized(existingAddress.address_line) ===
          normalized(nextAddressPayload.address_line) &&
        normalized(existingAddress.postal_code) ===
          normalized(nextAddressPayload.postal_code);
      ...
      if (shouldUpdateDefault) {
        ...
        toast.success(
          checkoutText(
            "checkout.addressUpdated",
            "Saved shipping address updated"
          )
        );
      } else {
        const { error: insertError } = await supabase.from("addresses").insert({
    ...
    try {
      ...
      const paystackInitPayload = {
        ...
        metadata: {
          ...
          offerValue:
            finalAppliedOffer?.value ??
            (legacyCouponPercent > 0 ? String(legacyCouponPercent) : undefined),
      ...
      const initResponse =
        await trpcClient.paystack.transactions.initialize.mutate(
          paystackInitPayload
        );
  ...
  const handleStripePayment = async () => {
    ...
    try {
      ...
      if (validItems.length === 0) {
        throw new Error(
          "No valid items in cart. Please refresh and try again."
        );
      }
      ...
      await redirectToStripeCheckout({
        ...
        email: sanitizeEmail(formData.email, 255),
        userName:
          sanitizeName(formData.firstName, 60) +
          " " +
          sanitizeName(formData.lastName, 60),
        items: validItems,
    ...
    <div className="min-h-screen bg-white w-full overflow-x-hidden">
      <div className="max-w-screen-xl mx-auto px-3 sm:px-4 lg:px-6 py-8 sm:py-12 lg:py-16">
        ...
        <div className="mb-6 sm:mb-8 md:mb-12 px-0">
          <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-2 sm:mb-3 text-black">
        ...
        <div className="grid gap-6 sm:gap-8 grid-cols-1 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 w-full min-w-0">
                ...
                <form className="space-y-5 sm:space-y-6">
                  ...
                  {isAuthenticated && (
                    <div className="p-4 rounded border border-gray-200 bg-gray-50 text-sm text-gray-700">
                      Shipping addresses are saved automatically for future
                      checkout. If you use a different address, you will be
                      asked whether to update your saved address.
                    </div>
                  )}
            ...
            {step === "review" && (
              <div className="space-y-4 sm:space-y-6 w-full">
                ...
                <div className="bg-white border border-gray-200 rounded p-4 sm:p-6 md:p-8 w-full">
                  <h2 className="text-xl sm:text-2xl font-bold mb-6 sm:mb-8 text-black">
                    {t("checkout.reviewYourOrder", "Review Your Order")}
                  </h2>
                  ...
                  <div className="py-6">
                    <h3 className="font-bold text-black mb-4">
                      {t("checkout.items", "Items")} ({cartItems.length})
                    </h3>
          ...
          <div className="lg:col-span-1 w-full min-w-0">
            <div className="bg-white border border-gray-200 rounded p-4 sm:p-6 md:p-8">
              <h3 className="text-base sm:text-lg font-bold mb-6 sm:mb-8 text-black">
                    ...
                    <div key={idx} className="flex gap-3">
                      {item.image ? (
                        <img
                          ...
                          onError={e => {
                            e.currentTarget.src =
                              "data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22%3E%3Crect fill=%22%23f3f4f6%22 width=%22100%22 height=%22100%22/%3E%3Ctext x=%2250%22 y=%2255%22 text-anchor=%22middle%22 fill=%22%239ca3af%22 font-size=%2220%22%3ENo image%3C/text%3E%3C/svg%3E";
                          }}
                        />
                      ) : (
                ...
                <div className="mb-4">
                  ...
                  <div className="mt-2 flex gap-2">
                    <input
                      ...
                      onChange={e => {
                        setCouponCodeInput(e.target.value.trim());
                        setCouponError(null);
                      }}
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          if (!Array.isArray(body) || body.length === 0) {
                            ...
                            setCouponError(
                              "Coupon is not valid or not applicable to your order"
                            );
                            return;
                          }
                          ...
                          if (!offer.active) {
                            ...
                            offer.minimumSubtotal &&
                            subtotal < Number(offer.minimumSubtotal)
                          ) {
                            setCouponError(
                              `Coupon requires minimum subtotal of ${currencyClient.getCurrencySymbolLocal()}${currencyClient.convertUSD(Number(offer.minimumSubtotal)).toFixed(2)}`
                            );
                            setAppliedOfferData(null);
                            setAppliedOfferDiscount(0);
                            setAppliedCouponLabel(null);
                ...
                <div className="flex gap-3 items-start">
                  ...
                  <span>
                    {t(
                      ...
                      `Free shipping on orders over ${currencyClient.getCurrencySymbolLocal()}${currencyClient.convertUSD(getFreeShippingThresholdUsd()).toFixed(2)}`
                    )}
                  </span>
                </div>
                <div className="flex gap-3 items-start">
                  ...
                  />
                  <span>
                    {t("checkout.returnPolicy", "30-day return policy")}
                  </span>
                </div>
                <div className="flex gap-3 items-start">
                  <Check
                  ...
                  />
                  <span>
                    {t("checkout.securePayments", "Secure encrypted payments")}
                  </span>
                </div>
              </div>
            </div>
