...
export async function setupVite(app: Express, server: Server) {
  ...
  if (process.env.NODE_ENV !== "development") {
    logger.warn(
      "[Vite] setupVite called in non-development mode, skipping vite setup"
    );
    return;
  }
  ...
  try {
    ...
    app.use("*", async (req, res, next) => {
      ...
      try {
        ...
        const render = ssrModule.render as
          | ((requestUrl: string) => Promise<{ html: string; head: string }>)
          | undefined;

        if (typeof render !== "function") {
          throw new Error(
            "SSR render function not found in /src/entry-server.tsx"
          );
        }
    ...
    logger.error({ data: [err?.message] }, "[Vite] Failed to set up Vite:");
