...
export const appRouter = router({
    ...
    syncOAuthUser: publicProcedure
      .input(
        z.object({
          ...
          loginMethod: z
            .enum(["google", "github", "apple", "microsoft", "email"])
            .default("google"),
        })
      )
  ...
  cart: router({
    ...
    addItem: protectedProcedure
      ...
      .mutation(({ ctx, input }) =>
        addToCart(ctx.user.id, input.productId, input.variantId, input.quantity)
      ),
  }),
  ...
  wishlist: router({
    getWishlist: protectedProcedure.query(({ ctx }) =>
      getUserWishlist(ctx.user.id)
    ),

    addItem: protectedProcedure
      .input(z.object({ productId: z.number() }))
      .mutation(({ ctx, input }) =>
        addToWishlist(ctx.user.id, input.productId)
      ),

    removeItem: protectedProcedure
      .input(z.object({ productId: z.number() }))
      .mutation(({ ctx, input }) =>
        removeFromWishlist(ctx.user.id, input.productId)
      ),
  }),
    ...
    create: protectedProcedure
      ...
      .mutation(({ ctx, input }) =>
        createOrder(
          ...
          undefined,
          input.language ?? ctx.req.header("accept-language") ?? undefined
        )
      ),
  }),
  ...
  notifications: router({
    create: protectedProcedure
      ...
      .mutation(({ ctx, input }) =>
        createNotification(ctx.user.id, input as any)
      ),
      ...
      .query(async ({ input, ctx }) => {
        try {
          ...
          const scored = allProducts.map((p: any) => {
            ...
            if (brandText && manufacturer && brandText.includes(manufacturer))
              score += 20;
          ...
          try {
            const sessionId =
              (ctx.req.header && ctx.req.header("x-session-id")) ||
            ...
            await recordProductSearchTrackingEvent({
              ...
              userAgent: ctx.req.header
                ? ctx.req.header("user-agent") || null
                : null,
            });
          } catch (trackErr) {
